...
import { Bank, CreditCard, Edit } from "iconoir-react";
...
const BankAccountForm = (): JSX.Element => {
  const params = useParams();
  const clientParam = params as { clientId: string };

  function action() {
    updateClient<BankAccountDataForm>(clientParam.clientId, formData);
  }

  const { loading, client, updateClient } = useClientsStore();

  function fillForm(): BankAccountDataForm {
    if (client)
      return {
        accountType: client.bankAccount.accountType,
        bank: client.bankAccount.bank,
        name: client.bankAccount.name,
        bankingApplication: client.bankAccount.bankingApplication,
        transfers: client.bankAccount.transfers,
        accountNumber: client.bankAccount.accountNumber,
      };
